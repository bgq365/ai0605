        ...
        <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="grid gap-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <ActionButton disabled={isPending || editableRows.length === 0 || blockingIssues.length > 0} onClick={handleCommit} primary>
                {isPending ? "提交中..." : "提交下单"}
              </ActionButton>
            </div>
          </div>
          ...
          <div className="rounded-[24px] bg-[#f6fbfb] p-4 text-sm text-[#335b60]">
            ...
            {suggestion ? (
              <div className="mt-4 rounded-2xl border border-[#d2eceb] bg-[#fbfefe] p-4 text-sm text-[#335b60]">
                ...
                <p className="mt-2">高置信字段数：{Object.values(suggestion.confidenceByField).filter((value) => value >= 0.85).length}</p>
              ...
              <div className="max-h-[540px] overflow-auto">
                <table className="min-w-full border-collapse text-sm">
                  <thead className="sticky top-0 z-10 bg-[#eefafa]">
                    <tr>
                      <th className="border-b border-[#d6eceb] px-3 py-3 text-left font-semibold text-[#0b6e6e]">操作</th>
                    ...
                    {editableRows.map((row) => {
                      ...
                      return (
                        <tr key={row.id} className="border-b border-[#edf4f4] align-top">
                          ...
                          {editableColumns.map((column) => {
                            const fieldIssues = issuesForRow.filter((issue) => issue.field === column.key || issue.field === "recipientGroup");
